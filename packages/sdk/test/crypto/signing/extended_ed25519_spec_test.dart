...
import 'package:plasma_sdk/src/crypto/generation/bip32_index.dart';
...
import 'package:plasma_sdk/src/crypto/signing/extended_ed25519/extended_ed25519_spec.dart'
    as x_spec;
import 'package:plasma_sdk/src/crypto/signing/signing.dart';
...
main() {
  group('Extended Ed25519 Topl test vectors', () {
    group('ed25519 spec tests', () {
      ...
      (
        x_spec.SecretKey secretKey,
        Uint8List message,
        x_spec.PublicKey verificationKey,
        Uint8List signature
        ...
        return (
          ...
          x_spec.PublicKey(
              spec.PublicKey(vk.sublist(0, 32)), vk.sublist(32, 64)),
    ...
    test(
        'With ExtendedEd25519, signed message should be verifiable with appropriate public key',
        () {
    ...
    test(
      ...
      final specOutSk = x_spec.SecretKey(
        "d8f0ad4d22ec1a143905af150e87c7f0dadd13749ef56fbd1bb380c37bc18c58"
            .toHexUint8List(),
        "a900381746984a637dd3fa454419a6d560d14d4142921895575f406c9ad8d92d"
            .toHexUint8List(),
        "cd07b700697afb30785ac4ab0ca690fd87223a12a927b4209ecf2da727ecd039"
            .toHexUint8List(),
      );

      final specOutVk = x_spec.PublicKey(
        spec.PublicKey(
            "e684c4a4442a9e256b18460b74e0bdcd1c4c9a7f4c504e8555670f69290f142d"
                .toHexUint8List()),
        "cd07b700697afb30785ac4ab0ca690fd87223a12a927b4209ecf2da727ecd039"
            .toHexUint8List(),
      );
    ...
    group('ed25519 Child Key Derivation tests', () {
      for (final x in ckdEd25519Vectors) {
        ...
        test("Child Key Derivation: ${vector.description}", () {
          ...
          final dChildKeyPair = xEd25519.deriveKeyPairFromChildPath(
              vector.rootSecretKey, vector.path);

          final dChildXSK = vector.path.fold(vector.rootSecretKey,
          ...
          final dChildXVK = vector.rootVerificationKey.map((vk) => vector.path.fold(
              ...
              (xvk, ind) => ind is SoftIndex
                  ...
                  : throw Exception(
                      'Received hardened index when soft index was expected')));
