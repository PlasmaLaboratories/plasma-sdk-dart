...
import 'package:plasma_sdk/src/quivr/quivr.dart';
...
main() {
  group('Quivr Atomic', () {
    test(
        "A locked proposition must return an LockedPropositionIsUnsatisfiable when evaluated",
        () {
      final lockedProposition = Proposer.lockedProposer(Data());
      final lockedProverPoof = Prover.lockedProver();

      final result = Verifier.verify(lockedProposition, lockedProverPoof,
    ...
    test("A tick proposition must evaluate to true when tick is in range", () {
      ...
      final result = Verifier.verify(tickProposition, tickerProverProof,
          MockHelpers.dynamicContext(tickProposition, tickerProverProof));
    ...
    test("A height proposition must evaluate to true when height is in range",
      ...
      final result = Verifier.verify(heightProposition, heightProverProof,
    ...
    test(
        "A height proposition must evaluate to false when height is not in range",
        () {
      final heightProposition = Proposer.heightProposer(
    ...
    test(
        "A signature proposition must evaluate to true when the signature proof is correct",
        () {
      ...
      final signatureProposition = Proposer.signatureProposer(
          ...
          VerificationKey(ed25519: VerificationKey_Ed25519Vk(value: vk)));

      final signature = VerySecureSignatureRoutine.sign(
          sk, MockHelpers.signableBytes.value.toUint8List());
      final signatureProverProof = Prover.signatureProver(
          Witness(value: signature), MockHelpers.signableBytes);

      final result = Verifier.verify(
          signatureProposition,
          signatureProverProof,
          MockHelpers.dynamicContext(
              signatureProposition, signatureProverProof));

      expect(result.isRight, true);
    });

    test(
        "A signature proposition must evaluate to false when the signature proof is not correct",
        () {
    ...
    test(
        "A digest proposition must evaluate to true when the digest is correct",
        () {
      final salt = Uint8List.fromList(MockHelpers.saltString.codeUnits);
      final preImage = Preimage(
          input: Uint8List.fromList(MockHelpers.preimageString.codeUnits),
          salt: salt);

      final digest = Digest(
          value: Blake2b256()
              .hash(Uint8List.fromList(preImage.input + preImage.salt)));
      ...
      final digestProposition =
          Proposer.digestProposer(MockHelpers.hashString, digest);
      final digestProverProof =
          Prover.digestProver(badPreimage, MockHelpers.signableBytes);

      final result = Verifier.verify(digestProposition, digestProverProof,
    ...
    test("Proposition and Proof with mismatched types fails validation", () {
      final proposition = Proposer.heightProposer(
          MockHelpers.heightString, Int64(900), Int64(1000));
      final proof = Prover.tickProver(MockHelpers.signableBytes);

      final result = Verifier.verify(
          proposition, proof, MockHelpers.dynamicContext(proposition, proof));
